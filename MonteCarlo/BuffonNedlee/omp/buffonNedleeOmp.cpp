...
void simulation(int n, int t, int d, puntos &xs, puntos &angulosradians,int factorHilos, int factorDinamico){ 
    double enlinea = 0.0;
    //int th_id;
    int p= omp_get_num_procs() * factorHilos;//2;

    #pragma omp parallel for num_threads(p) schedule(static, factorDinamico) reduction(+:enlinea)
...
int main(int argc, char *argv[]){
    ...
    int n = 0, t = 0, d = 0, factorHilos=2, factorDinamico=2;
    try
    {
        ...
        factorHilos = stoi(argv[4]);//argv 3, factor hilos
        factorDinamico = stoi(argv[5]);//argv 4, factor dinamico
        
    }
    catch(const exception& e)
    {
        std::cerr << "los valores ingresados estan mal" << '\n';
        n = 0; t = 0; d = 0; factorHilos=2; factorDinamico=2;
    }
  ...
  start = std::chrono::system_clock::now();
    ...
    simulation(n,t,d,xs,angulosRadians,factorHilos,factorDinamico);
